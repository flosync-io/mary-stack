...
- **Apply/publish builds on the BUCKET base, never the local file.** The bucket `knowledge/<machine>.json` is authoritative (Mary reads it; promotions land there first). The git-tracked `knowledge/<machine>.json` can lag (it did: pre-`_meta`, missing prior ENRICH provenance). Fold a new version onto the bucket copy (`storage.get_knowledge`) so earlier promotions aren't clobbered, publish, then reconcile the local file from the published version and commit it as the version archive.
- **ADD merge inserts only the failure_mode.** A real ADD usually drags new `machine_components`/`checks`/`procedures`/`steps` the FM references; `promote.py --apply` does not add those. Today they're folded by a one-shot script reusing `load_completed_draft` (the gate) + `bump_version` + `storage`. `component_ref`/`confirm_checks`/`procedures`/`steps` are string refs — schema does NOT enforce they resolve, so a merged FM can carry dangling refs; author the dependency objects in the same version bump.
